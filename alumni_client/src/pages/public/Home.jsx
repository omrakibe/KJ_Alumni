import { ArrowRight, BriefcaseBusiness, CalendarDays, Globe2, Sparkles } from "lucide-react";
...
function Home() {
  ...
  useEffect(() => {
    let active = true;
    getPublicEvents({ page: 0, size: 3 })
      .then(({ data }) => { if (active) setEvents(data.data?.content || []); })
      .catch(() => { if (active) setEventsError("Upcoming events could not be loaded. Please try again shortly."); })
      .finally(() => { if (active) setEventsLoading(false); });
    return () => { active = false; };
  }, []);
  return <div className="home">
    <nav className="public-nav"><div className="container public-nav-inner"><Link to="/" className="public-brand"><span className="public-brand-mark"><img src={collegeLogo} alt="KJCOEMR" /></span><span>KJCOEMR <small>Alumni Network</small></span></Link><div className="public-nav-links"><a href="#community">Community</a><a href="#events">Events</a><Link to="/login">Sign in</Link><Link to="/register" className="public-nav-cta">Join the network <ArrowRight size={16} /></Link><ThemeToggle className="theme-toggle-public" /></div></div></nav>

    <main>
      <section className="public-hero"><img src={campusImage} alt="KJCOEMR campus" className="public-hero-image" /><div className="public-hero-overlay" /><div className="container public-hero-content"><div className="public-hero-copy"><p className="public-eyebrow"><Sparkles size={14} /> KJCOEMR Alumni Association</p><h1>Stay connected to the people who shaped your journey.</h1><p>One trusted place for KJCOEMR alumni to reconnect, share opportunities and celebrate what comes next.</p><div className="public-hero-actions"><Link to="/register" className="public-button public-button-primary">Join alumni network <ArrowRight size={18} /></Link><Link to="/login" className="public-button public-button-secondary">Alumni sign in</Link></div></div><aside className="public-upcoming-panel" aria-busy={eventsLoading}><div className="public-upcoming-title"><span><CalendarDays size={15} /> Upcoming events</span><a href="#events">View all</a></div>{eventsLoading ? <div className="public-events-loading">Loading events…</div> : eventsError ? <div className="public-events-loading" role="alert">Events are temporarily unavailable.</div> : events.length ? <div className="public-upcoming-list">{events.slice(0, 3).map((event) => <article key={event.id}><p>{new Date(event.eventDateTime).toLocaleDateString(undefined, { day: "numeric", month: "short" })}</p><div><strong>{event.title}</strong></div></article>)}</div> : <><strong>No upcoming events</strong><small>New college and branch events will appear here.</small></>}</aside></div></section>
      ...
      <section className="public-events" id="events"><div className="container"><div className="public-events-heading"><div><p className="public-section-label">Campus calendar</p><h2>Upcoming alumni events</h2><p>Join the KJCOEMR alumni network to RSVP and stay connected.</p></div><Link to="/login" className="public-text-link">Sign in to RSVP <ArrowRight size={16} /></Link></div><div className="public-events-grid" aria-busy={eventsLoading}>{eventsLoading ? <div className="public-events-empty">Loading upcoming events…</div> : eventsError ? <div className="public-events-empty public-events-error">{eventsError}</div> : events.length ? events.map((event) => <article key={event.id}><div className="public-event-icon"><CalendarDays size={20} /></div><p>{new Date(event.eventDateTime).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}</p><h3>{event.title}</h3></article>) : <div className="public-events-empty">No alumni events are scheduled right now. Please check back soon.</div>}</div></div></section>
    </main>
