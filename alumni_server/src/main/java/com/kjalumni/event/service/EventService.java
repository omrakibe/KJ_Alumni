...
public class EventService {
    ...
    @Transactional(readOnly = true) public Page<EventResponse> adminEvents(User user, Pageable pageable) { validateAdmin(user); return (user.getBranch() == null ? repository.findAll(pageable) : repository.findVisibleToAdminBranch(user.getBranch(), user, pageable)).map(event -> map(event, null, user)); }
    @Transactional(readOnly = true) public Page<EventResponse> adminUpcomingEvents(User user, Pageable pageable) { validateAdmin(user); LocalDateTime now = LocalDateTime.now(); return (user.getBranch() == null ? repository.findByStatusAndEventDateTimeGreaterThanEqualOrderByEventDateTimeAsc(EventStatus.ACTIVE, now, pageable) : repository.findUpcomingVisibleToAdminBranch(user.getBranch(), user, EventStatus.ACTIVE, now, pageable)).map(event -> map(event, null, user)); }
    @Transactional(readOnly = true) public Page<EventResponse> adminEventHistory(User user, Pageable pageable) { validateAdmin(user); LocalDateTime now = LocalDateTime.now(); return (user.getBranch() == null ? repository.findByStatusAndEventDateTimeLessThanOrderByEventDateTimeDesc(EventStatus.COMPLETED, now, pageable) : repository.findHistoryVisibleToAdminBranch(user.getBranch(), user, EventStatus.COMPLETED, now, pageable)).map(event -> map(event, null, user)); }
    ...
    @Transactional public void cancel(UUID id, User user) {
        AlumniEvent event = findManageable(id, user);
        List<String> rsvpEmails = rsvpRepository.findRecipientEmailsByEvent(event);
        rsvpRepository.deleteAllByEvent(event);
        repository.delete(event);
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            @Override public void afterCommit() {
                eventNotificationService.notifyEventCancellation(event, rsvpEmails);
            }
        });
    }
    @Transactional(readOnly = true) public EventResponse adminEvent(UUID id, User user) { return map(findAdminReadable(id, user), null, user); }
    ...
    @Transactional(readOnly = true) public Page<EventResponse> publicEvents(Pageable pageable) { return repository.findByStatusAndEventDateTimeGreaterThanEqualOrderByEventDateTimeAsc(EventStatus.ACTIVE, LocalDateTime.now(), pageable).map(this::publicMap); }
    ...
    private AlumniEvent findManageable(UUID id, User user) { AlumniEvent event = findAdminReadable(id, user); if (!canManage(event, user)) throw new AccessDeniedException("You are not authorized to manage this event."); return event; }
    private AlumniEvent findAdminReadable(UUID id, User user) { validateAdmin(user); AlumniEvent event = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Event not found.")); if (user.getBranch() != null && event.getVisibility() != EventVisibility.ALL && event.getBranch() != user.getBranch() && !event.getCreatedBy().getId().equals(user.getId())) throw new AccessDeniedException("You are not authorized to view this event."); return event; }
    private boolean canManage(AlumniEvent event, User user) { return user.getBranch() == null || event.getCreatedBy().getId().equals(user.getId()) || event.getVisibility() == EventVisibility.BRANCH && event.getBranch() == user.getBranch(); }
    ...
    private EventResponse map(AlumniEvent event, User alumniUser) { return map(event, alumniUser, null); }
    private EventResponse map(AlumniEvent event, User alumniUser, User adminUser) { return EventResponse.builder().id(event.getId()).title(event.getTitle()).description(event.getDescription()).eventDateTime(event.getEventDateTime()).venue(event.getVenue()).branch(event.getBranch()).visibility(event.getVisibility()).status(event.getStatus()).createdBy(event.getCreatedBy().getId()).attendeeCount(alumniUser == null ? rsvpRepository.countByEvent(event) : null).rsvped(alumniUser != null && rsvpRepository.existsByEventAndAlumniUser(event, alumniUser)).canManage(adminUser != null && canManage(event, adminUser)).build(); }
    private EventResponse publicMap(AlumniEvent event) { return EventResponse.builder().id(event.getId()).title(event.getTitle()).description(event.getDescription()).eventDateTime(event.getEventDateTime()).venue(event.getVenue()).branch(event.getBranch()).visibility(event.getVisibility()).status(event.getStatus()).createdBy(event.getCreatedBy().getId()).build(); }
